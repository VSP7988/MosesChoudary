...
const ManageFounderGallery = () => {
  ...
  const uploadImage = async (file: File) => {
    ...
    const filePath = `founder-gallery/${fileName}`;

    const { error: uploadError } = await supabase.storage
      .from('founder-gallery')
  ...
  const handleDeleteImage = async (id: string, imageUrl: string) => {
    ...
    try {
      ...
      await supabase.storage
        .from('founder-gallery')
        .remove([`founder-gallery/${fileName}`]);
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold mb-6">Manage Founder Gallery</h2>
...
export default ManageFounderGallery;