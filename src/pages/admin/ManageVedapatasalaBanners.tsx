...
const ManageVedapatasalaBanners = () => {
  ...
  const uploadImage = async (file: File) => {
    ...
    const filePath = `vedapatasala-banners/${fileName}`;

    const { error: uploadError } = await supabase.storage
      .from('vedapatasala-banners')
  ...
  const handleDeleteBanner = async (id: string, imageUrl: string) => {
    ...
    try {
      ...
      await supabase.storage
        .from('vedapatasala-banners')
        .remove([`vedapatasala-banners/${fileName}`]);
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold mb-6">Manage Vedapatasala Banners</h2>
...
export default ManageVedapatasalaBanners;